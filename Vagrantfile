...
Vagrant.configure("2") do |config|
    ...
    # config.vm.synced_folder "share", "/home/vagrant/share", type: "smb"

    vmconfig['shared_folders'].each do |value|
        if value['type'] == 'nfs'
            config.vm.synced_folder value['host_path'],
                value['guest_path'],
                create: true,
                type: "nfs"
        elsif value['type'] == 'smb'
            config.vm.synced_folder value['host_path'],
                value['guest_path'],
                create: true,
                type: "smb",
                smb_username: value.fetch('smb_username', nil),
                smb_password: value.fetch('smb_password', nil)
        end
    end
